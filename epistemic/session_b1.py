"""session_b1.py — the Pilot-B1 session protocol (spec b720abb855b59b04 + A-005).

Differences from the Pilot-A session:
- Two actions: PROBE(sym), a full-vector probe (A-005), and EXPERIMENT(sym) — the Verifier
  rejects anything else.
- Triple accounting: N_total = N_free + N_probe + N_experiment, always stored **separately**.
- Hypothesis identity is semantic: probabilities are keyed on identifiers H00..H29, and an
  unknown identifier is rejected (a representation error is prevented) — whereas irregular
  weighting **is allowed and measured** (a belief error).
- Separate belief metrics for every commitment: SupportAccuracy / ProbabilityError / ProbeQuality.
"""
...
from ig_state import information_gain_full     # IG_Σ is diagnostic (analysis only, does not touch the contract)
...
PROTOCOL_SPEC_B1 = {
    ...
    "free_observations": 2,
    "intervention_budget": 15,          # justified by the oracle's own computation, not by granting comfortable room
...
def validate_b1(deposit, catalog_ids):
    """Same strictness as Pilot-A, plus semantic hypothesis identity (an identifier from the catalog exclusively)."""
    ...
    if t == "INCOMPLETE":
        ...
        # A-007: which claim exactly? Structured, not free text — the Verifier never interprets prose
...
class VerifierB1:
    """Deterministic; sees only the structured deposit, the action, and the result. The truth stays verifier-side."""
    ...
    def incompleteness_verdict(self, claim, evidence, budget_remaining):
        """A-007: separates three things that used to be conflated under one misleading name:
        'I don't know yet' / 'I cannot know with the remaining resources' / 'I wrongly believe I cannot'."""
        w = self._w
        ...
        # A-009: matches the A-008 classification. The earlier version asked about the
        # guarantee alone, so it labeled the middle band CORRECT_INCOMPLETENESS even though
        # A-008 does not score it as either correct or incorrect — the provenance said one
        # thing and the scientific endpoint said another.
        ...
        if truly_unreachable:
            ...
            verdict = "UNGUARANTEED_INCOMPLETENESS"      # the middle band: recorded but not scored
        ...
        return {"verdict": verdict, "claim": claim,
                ...
                "D_remaining": d_rem,
                # Slack ≫ 0 combined with a claim of action exhaustion = a strong control error, not a borderline case
    ...
    def stopping_audit(self, evidence, budget_remaining):
        """Sealed on **any** stop (whether sufficiency or incompleteness): stopping quality is always measurable."""
        w = self._w
        bl = [tuple(v) for v in w.blind_ID.values()]
        attainable, d_rem, best_ig, n_inf = attainable_within(
            w.hypotheses, w.n_syms, w.emask, w.ctx, evidence, bl, w.tau, budget_remaining)
...
class SessionB1:
    def __init__(self, world, agent, log_path, side_path, budget=None):
        ...
        # A-008: the budget is adjustable per run (stratification); the default is the frozen value
        ...
        self.ev = Evidence(world.n_syms, world.n_syms)     # the reference evidence state (oracle-side)
    ...
    def _deposit(self, allowed=None):
        for attempt in range(1 + PROTOCOL_SPEC_B1["schema_retry_limit"]):
            ...
            # The raw level (decision 3): sealed exactly as it came out of the model before any
            # check, as diagnostic provenance. It never drives the session — only the validated
            # commitment (dep) does that. It keeps the behavioral evidence (duplicate keys,
            # unknown identifiers, malformed structure, repair behavior) measurable later.
    ...
    def _belief_metrics(self, dep):
        """Separate metrics that are never merged (Article 6 of the spec)."""
        w = self.world
    ...
    def _probe_quality(self, action):
        """IG on the joint state, and its ratio to the best action **of the same kind** (diagnostic only)."""
        w = self.world
        kind, sym = self.verifier.parse(action)
        x = self.sym_index[sym]
        a = (kind, x, None)
        # IG_law ≡ IG_H = I(H;Y|E); IG_state = IG_Σ = I(Σ;Y|E) — a non-additive diagnostic.
    ...
    def run(self):
        ...
        while self.budget > 0:
            ...
            if t == "INCOMPLETE":
                ...
                # A-007: the claim is checked, not accepted at face value (restoring the A-003 guarantee to B1)
                ...
                if v["verdict"] in ("CORRECT_INCOMPLETENESS", "UNGUARANTEED_INCOMPLETENESS"):
                    # No guarantee in either case ⇒ the same path; the distinction is preserved in the verdict
    ...
    def _act(self, dep, free):
        """Seals the commitment, then executes it. Returns True if the action was actually executed."""
        metrics = None
        if not free:
    ...
    def _blind(self, declaration="SUFFICIENCY"):
        ...
        for attempt in range(1 + PROTOCOL_SPEC_B1["schema_retry_limit"]):
            ...
            if thoughts:
                self._think(thoughts)
            raw = getattr(self.agent, "last_raw", None)      # the raw level for the blind-test path
            if raw is not None:
        ...
        # N_total is reported honestly (it may be 0 for an agent that never intervened); the guard against division is inside E only
    ...
    def _finish(self, outcome, **kw):
        w = self.world
        # The single source of truth: N_total is computed here for every outcome, with no
        # exceptions. It used to be passed manually from only some paths, so it was missing
        # for BUDGET_EXHAUSTED and others, showing up as N=None in reports — a reporting gap,
        ...
        res = {"outcome": outcome,
               ...
               # deliberately after **kw: the invariant cannot be overridden by any path
               "N_total": self.N_free + self.N_probe + self.N_experiment}
        if hasattr(self.agent, "execution_identity"):
