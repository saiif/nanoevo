"""analyze_replication.py — closes out R-001 on 4010-4019 using the pinned definitions (commit 2441332).

Reuses audit_b1_small.audit_seed and audit_b1_efficiency.analyze verbatim (the same
definitions of F, E_only, IG, cost identity), redirected to the replication folder only.
No redefinition. Then judges F1/F2/F3 against the falsification conditions frozen in
R-001, and computes our own Brier score.
"""
...
from audit_b1_small import audit_seed          # noqa: E402 — after redirecting RUN
...
def main():
    ...
    # Correction: the prediction threshold is not the falsification boundary. The previous
    # version used (n_solved >= 6) — the **falsification boundary** — as the HELD condition,
    # so the stated prediction (>=8/10) could "succeed" at 6/10, and that boolean then fed
    # into the Brier score. The Brier score must be scored on the prediction, not on the death boundary.
    f1_predicted = (n_solved >= 8) and (median_blind == 1.0)      # the prediction recorded verbatim
    f1_falsified = (n_solved < 6)                                  # the recorded falsification boundary
    f1 = f1_predicted                                              # what feeds into the Brier score
    ...
    # Brier score on us (predicted probability against a binary outcome, held=1)
    preds = [("F1", 0.85, f1), ("F2", 0.80, bool(f2)), ("F3", 0.75, f3)]
    # Scored on whether the **predictions** held, not on crossing the falsification boundaries
