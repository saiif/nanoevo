"""run_mechanical.py — mechanical validation (Appendix B): does the machine do what the paper claims?

Seven mechanical questions + violation table (A.3) + A-003 rules, all PASS/FAIL.
No statistical claims. S_mechanical = seeds 1..20 (confirmatorily burned forever under A.2).

Output: provenance/mech_run/ (the latest run); any prior run is archived to
provenance/mech_run_archive_<ts>/ — a failure that led to a fix is never erased (A-001).
"""
...
def main():
    ...
    # provenance: we don't erase a failure that led to a fix — we archive it (A-001)
    ...
    # ---------- Q7: D_min correct (independent recomputation + manually checkable) ----------
    ...
    # ---------- Q1: no leakage — hidden info does not appear in any payload the agent sees ----------
    ...
    for rec in s1.arch.records():
        if rec["payload"]["kind"] in agent_visible_kinds:
            ...
            if any(m in blob for m in secret_markers):
                leaked = True
    # and behavioral test: an out-of-region action is refused before execution
    ...
    # ---------- Q2: the gate — no execution without a valid sealed deposit ----------
    ...
    for rec in s1.arch.records():
        if rec["payload"]["kind"] == "OBSERVATION":
            ...
            gate_ok &= len(prior) == 1  # every observation is tied to a prior sealing receipt
    check("Q2 every observation traces to a prior sealed commitment", gate_ok)

    # ---------- Q3: the blind test is actually blind ----------
    ...
    # ---------- Q4: chain integrity + tampering detection ----------
    ...
    # ---------- Q6: incompleteness/sufficiency declarations pass through the channel unambiguously ----------
    ...
    # ---------- A-003: hardening the contract before Pilot-A ----------
    ...
    # the correct case: does not appear in an accepted world (every class is singleton) — tested on a rejected world
    correct_case = None
    for k in range(200):
    ...
    # ---------- A-004: the four rulers and the task-aligned audit rule ----------
    print("\n[A-004] Four rulers and the task-aligned audit rule")
    ordering_ok, floors_ok = True, True
    for seed in range(1, 13):
        w = generate_accepted_world(seed)
        # structural ordering: floor^task <= floor^strict, and floor^task <= D_robust
    ...
    # the three-way classification is deterministic and exhaustive
    ...
    # the task floor is actually weaker than the strict floor somewhere (otherwise the distinction is moot)
    ...
    # the reference agent never falls below the task floor (otherwise the bug is in the ruler or the computation)
    audit_free = True
    ...
    # E is uncapped: a path shorter than the guarantee gives E>1 instead of being hidden by min(1,.).
    # the reference Learner never drops below D_robust (it seeks full identification), so we use the early-stop bot.
    ...
    # ---------- Learner vs Control display (no statistics — plumbing only) ----------
    ...
    # ---------- the tally ----------
    # adversarial checks on the contract (A-009): each one was a real vulnerability found in code review
