"""audit_b1_small.py — post-hoc audit of B1-small sessions (seeds 4000..4009).

Analysis only, over the sealed logs: reconstructs the world from the seed, replays the
evidence path, and computes (IG_joint, IG_H, IG_Σ) for every commitment — because the
recorded rows predate IG_Σ. Does not touch the contract or the oracle.

The separation this reveals: an EXPERIMENT on a symbol that has **not been probed**
gains state information (high IG_Σ), not law information (IG_H ~ 0); and on a symbol
that **has been probed**, the opposite holds. In other words, "experiment before probe"
can be a rational state-acquisition move.
"""
...
def audit_seed(seed):
    ...
    steps, seq = [], []
    pending = None                       # (kind, x, was_probed_before) awaiting its result
    for r in recs:
        p = r["payload"]
        k = p["kind"]
        if k in ("FREE_OBS_COMMIT", "COMMITMENT"):
            ...
            x = sidx[sym]
            probed_before = bin(ev.pmask[x]).count("1") == 1     # has the symbol's vector become known?
...
def main():
    ...
    for seed in range(4000, 4010):
        ...
        for s in steps:
            ...
            cpl = round(s["IG_joint"] - igh - igs, 4)         # coupling surplus (super-additive)
            # sanity check: the recomputed IG_joint and IG_H == the recorded ones (same oracle function)
            if s["rec_joint"] is not None and abs(s["IG_joint"] - s["rec_joint"]) > 1e-3:
                mismatches += 1
            if s["rec_law"] is not None and abs(igh - s["rec_law"]) > 1e-3:
                mismatches += 1
            # the regime is derived from the values, not from the action type (a probe after an experiment can still distinguish the law)
