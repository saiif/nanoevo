"""check_ig_state.py — mechanical validation of IG_Σ before relying on it (similar to run_mechanical_b1)."""
...
def main():
    ...
    # (1) Z from state_entropy == Z from posterior (same shared joint normalizer) over random evidence
    worstZ, tested = 0, 0
    for _ in range(150):
        n = rng.randint(2, 5)
    ...
    # (2) Initial state with no evidence: H(Σ) == log2(n!)
    okstart = True
    for n in range(2, 6):
        declared = rng.sample(ALL_VECTORS, n)
        ctx = Ctx(n, _subsets_by_popcount(n), declared, {})
    ...
    # (3) After pinning all symbols: H(Σ)==0 and IG_Σ for any experiment == 0
    okpin = True
    for _ in range(40):
        n = rng.randint(2, 5)
        declared = rng.sample(ALL_VECTORS, n)
        ...
        perm = list(range(n))
        rng.shuffle(perm)
        for x in range(n):                       # pin all assignments via probes
    ...
    # (4) IG_joint/IG_law from information_gain_full == from world_b1.information_gain literally
    okmatch = True
    for _ in range(80):
        n = rng.randint(2, 5)
        declared = rng.sample(ALL_VECTORS, n)
        ...
        if act[0] == "EXPERIMENT" and ev.exp[act[1]] is not None:
            okmatch &= j1 == 0.0 and l1 == 0.0 and s1 == 0.0     # redundant action ⇒ zeros
        else:
