"""run_mechanical_b1.py — mechanical validation for Pilot-B1 (before any LLM run).

The riskiest area here is **the ruler itself**: the bijection prior and the new minimax. If we
got them wrong, the agent would look calibrated or uncalibrated because of a wrong oracle — the
same class of errors Pilot-A taught us not to tolerate. That's why the first two checks prove the
ruler by explicit enumeration, not by reasoning about the code.
"""
...
# ---------- the ruler: explicit enumeration versus the permanent ----------

def brute_force_posterior(hyps, declared, n, evidence):
    """P*(h|E) by explicitly enumerating **all** bijections — a ruler fully independent of the permanent."""
    W = []
    for h in hyps:
        cnt = 0
...
def main():
    ...
    # ---------- B1-Q1: the posterior = explicit enumeration of bijections ----------
    ...
    for trial in range(120):
        ...
        ev = Evidence(n, n)
        for _ in range(rng.randint(0, 4)):        # random evidence
    ...
    # ---------- B1-Q2: symbol-permutation invariance (basis for the canonical search key) ----------
    ...
    # ---------- B1-Q4: one action model across every layer (A-005) ----------
    ...
    # ---------- B1-Q5: semantic hypothesis identity ----------
    ...
    try:
        ...
        text_rejected = True
    dup_collapses = len({"H07": 0.5, "H07": 0.5}) == 1     # noqa: F601 — intended to prove the collapse
    ...
    # ---------- B1-Q6: exact floors and a robust reference within a justified budget ----------
    ...
    # ---------- B1-Q7: adversarial check — no solution without probing (representation leakage) ----------
    ...
    # ---------- B1-Q9: separating representation defect from belief defect ----------
    ...
    # ---------- B1-Q10: a malformed action is refused before execution ----------
    ...
    # ---------- B1-Q11: log integrity + no leakage ----------
    ...
    for r in recs:
        ...
        if p["kind"] == "OBSERVATION":
            # an experiment result = a single bit only, not a vector
    ...
    # ---------- B1-Q12: A-007 — the three-way verdict on the incompleteness declaration ----------
    ...
    # B1-Q13: the stopping audit is actually sealed (a prior silent patch defined the function but never called it)
    ...
    # B1-Q14: N_total is present and consistent in **every** outcome (it used to be missing from BUDGET_EXHAUSTED)
    _seen_outcomes, _bad = {}, []
