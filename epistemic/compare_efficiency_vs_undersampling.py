"""compare_efficiency_vs_undersampling.py — the boundary between efficiency and recklessness.

Read-only analysis over the two completed batches:
    4000-4009  (10/10 solved)      vs   4010-4019  (8/10 + 2 FALSE_CERTAINTY)

The question: the successful N=8 sessions and the two failed sessions are both
"probe-light". Is the difference **economy** or **evidence shortfall**? The hypothesis
under test:

    Epistemic efficiency  ≠  Epistemic under-sampling
    A good agent does not gather the least information possible, but **the least
    information that is sufficient**.

The decisive test: the state of the reference posterior **at the moment of stopping**.

Important later update (do not read this file's output in isolation from it): it turned
out that 4015/4018 did not declare sufficiency but **INCOMPLETE**, and that each one's
posterior **matched the oracle exactly**. So the failure was neither under-sampling nor
a flawed belief about the world, but rather **a false belief about the remaining action
space** — see relabel_b1_incompleteness.py and A-007. This file remains valid for the
cost structure; the explanation of the failure comes from that one instead.

Does not touch the contract or the oracle, and does not redefine any pinned metric.
"""
...
def state_at_declaration(seed, run_dir):
    """Replays the evidence path up to the sufficiency declaration, and reads the reference posterior there.

    This is the decisive test: how many hypotheses were still alive at the moment the agent said "enough"?
    """
...
def structure(steps):
    """F / E_only / P_only, using the same definitions as commit 2441332 (no redefinition)."""
