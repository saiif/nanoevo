"""audit_b1_efficiency.py — why are some B1-small paths more efficient? (the structure of information, not its amount).

Builds on audit_b1_small.audit_seed: for each seed, classifies every action into its
information regime (state/law/synergy/redund) and measures the efficiency structure,
then compares N cohorts (8 vs 11).

The hypothesis under test: efficiency (low N) arises from exploiting synergy (an
experiment that harvests coupling in a single action) and avoiding redundancy (a probe
on top of the effect of a prior experiment); not from the "P→E ordering" by itself.
"""
...
def analyze(seed):
    ...
    ig_joint_sum = 0.0
    hi_joint = 0                          # actions with IG_joint >= 1.5 (information-dense)
    probed, experimented = set(), set()   # which symbols received each kind of action (for the instrumentation structure)
    for s in steps:
    ...
    n_actions = len(steps)   # includes free observations — not "paid"                   # all recorded actions (including free ones)
    both = probed & experimented          # a symbol fully instrumented (probe + experiment)
    exp_only = experimented - probed      # a law extracted by an experiment on a symbol that was not probed (synergy)
    probe_only = probed - experimented    # a probe not followed by an experiment = explicit waste
    return {
...
def main():
    ...
    # Cohort comparison: the more efficient (N<=9) vs the slower (N>=11)
