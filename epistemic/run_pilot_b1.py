"""run_pilot_b1.py — Pilot-B1-small: 10 worlds from S_pilot via the shim.

An **engineering/exploratory behavior test, not a performance study**: A-005 is not changed, nor the Session, nor the
oracle during the batch, and the results are not used to pick a "better" version of the contract.

The pre-registered halt rule:
- A **contract/instrument defect** (leakage, oracle inconsistency, chain/provenance failure, a semantic mismatch between the adapter
  and the contract) ⇒ halt the batch and investigate.
- **The model's own behavior** (a poor distribution, overusing probes, Blind-ID failure, general incompetence)
  ⇒ no halt and no fix. This is exactly what is being measured.

Usage: python run_pilot_b1.py [n] [model] [base_url]
"""
...
B1_SEED_BASE = 4020                    # S_pilot (>=2000); S_confirmatory stays sealed
HERE = os.path.dirname(os.path.abspath(__file__))
RUN_DIR = os.path.join(HERE, "provenance", "pilot_b1_a007_run")

# defects that halt the batch (instrument/contract) versus what does not halt it (model behavior)
INSTRUMENT_DEFECTS = ("SESSION_CRASH", "ORACLE_INCONSISTENT", "PROVENANCE_BROKEN")
# Note: INCORRECT_ACTION_EXHAUSTION and ACTIONABLE_INCOMPLETENESS are **not** instrument defects —
# they are exactly what the batch came to measure (agent behavior). They do not halt the batch.
...
def action_sequence(recs):
    """The raw P/E sequence as it actually occurred (includes free exploration)."""
    seq = []
    for r in recs:
        p = r["payload"]
...
def main(n=10, model="claude-sonnet-5", base_url="http://127.0.0.1:8787/v1"):
    ...
    for k in range(n):
        ...
        # A-007: halt diagnostic (oracle-side, not seen by the agent)
        ...
        # halt rule: instrument/contract defect only
