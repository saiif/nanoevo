"""analyze_b1_a007.py — four layers + ExcessActionsAfterSufficiency, across the three batches.

The two competing hypotheses after INCOMPLETE disappeared in batch A-007:

    H1: A-007 improved the agent's understanding of the action space (better control)
    H2: A-007 only changed the declaration policy — declaring incompleteness became
        costly so it is avoided, with no underlying improvement

Telling them apart does not require changing the contract: it is enough to ask **when**
the agent stopped relative to the moment success became guaranteed.

A subtle point: attainable_within is true **from the start** (accepted worlds are
solvable by construction), so it cannot serve as the marker for "success just became
guaranteed." The correct marker is the first step at which the criterion becomes
**actually satisfied** — i.e., the step at which stopping there would have succeeded.
This is the same counterfactual-correct termination condition used in D_robust: a
majority vote of the support achieves tau against **any** member, were that member the
true one.

    ExcessActionsAfterSufficiency = (actions taken until stopping) − (first step at which the criterion became satisfied)

Reading the result:
    excess ≈ 0 across batches   → control did not degrade; the disappearance of INCOMPLETE did not cost over-exploration
    excess rises after A-007    → H2: the error moved from premature stopping to over-exploration
"""
...
def criterion_met(w, support):
    """Is it enough to stop now? (counterfactual-correct: succeeds against any member, were it the true one)."""
    if not support:
        return True
...
def walk(seed, run_dir):
    """Replays the path step by step along with the moment the criterion is met."""
    ...
    return {"seed": seed, "run": run_dir, "outcome": (res or {}).get("outcome"),
            ...
            # The explicitly required fields: when the decision became possible, when it stopped, and what remained available then
